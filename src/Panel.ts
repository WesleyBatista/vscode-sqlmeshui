...
export class Panel extends EventEmitter2 {
  private static readonly viewType = 'sqlmeshui'
  ...
  public async createDebugPanel() {
    ...
    panel.on('focus', () => {
      commands.executeCommand('setContext', 'sqlmeshui-debug-active', true)
    })
    panel.on('blur', () => {
      commands.executeCommand('setContext', 'sqlmeshui-debug-active', false)
    })
    panel.once('disposed', () => {
      commands.executeCommand('setContext', 'sqlmeshui-debug-active', false)
